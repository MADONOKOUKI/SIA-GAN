...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        # block scrambling
#        x_stack = blockwise_scramble(imgs,perm_pair[batch_idx%len(perm_pair)][0])
 #       imgs1 = np.transpose(x_stack,(0,3,1,2))


  #      inputs1 = torch.from_numpy(imgs1)

   #     x_stack = blockwise_scramble(imgs,perm_pair[batch_idx%len(perm_pair)][1])
...
#        gen_labels[ : , perm[batch_idx%len(perm)]] =  1
 ...
 #       print(outputs.size())
   ...
   #     true_loss =  multi(perms_output, gen_labels) + mixup_criterion(criterion, outputs, targets_a, targets_b, lam) #+ multi(perms_output, gen_labels)
