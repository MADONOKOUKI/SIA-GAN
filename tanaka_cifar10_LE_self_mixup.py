...
perm = [(i,j) for i in range(args.num_of_keys) for j in range(args.num_of_keys)]
# Model
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        # block scrambling
        x_stack = blockwise_scramble(imgs,perm[batch_idx%len(perm)][0])
        imgs1 = np.transpose(x_stack,(0,3,1,2))


        inputs1 = torch.from_numpy(imgs1)

        x_stack = blockwise_scramble(imgs,perm[batch_idx%len(perm)][1])
        ...
        inputs, targets_a, targets_b, lam = mixup_data(inputs1, inputs2, targets,
                                                       1, True)
