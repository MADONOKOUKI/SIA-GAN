...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        inputs[:inputs.size()[0]//2], targets_a1, targets_b1, lam1 = mixup_data(inputs[:inputs.size()[0]//2], targets[:inputs.size()[0]//2], 1, True)

        inputs[:inputs.size()[0]//2], targets_a1, targets_b1 = map(Variable, (inputs[:inputs.size()[0]//2], targets_a1, targets_b1))


        inputs[inputs.size()[0]//2:], targets_a2, targets_b2, lam2 = mixup_data(inputs[inputs.size()[0]//2:], targets[inputs.size()[0]//2:], 1, True)

        inputs[inputs.size()[0]//2:], targets_a2, targets_b2 = map(Variable, (inputs[inputs.size()[0]//2:], targets_a2, targets_b2))
        ...
        for idx in range(2):
          lam_scr = np.random.beta(args.alpha, args.alpha)
     
          imgs = lam_scr * imgs[:inputs.size()[0]//2] + (1-lam_scr) * imgs[inputs.size()[0]//2:]
          inputs = torch.from_numpy(imgs)

          inputs, targets_a1, targets_b1, targets_a2, targets_b2  = inputs.cuda(), targets_a1.cuda(), targets_b1.cuda(), targets_a2.cuda(), targets_b2.cuda()

          optimizer.zero_grad()
          outputs, perms_output = net(inputs)
          true_loss =  lam_scr * mixup_criterion(criterion, outputs, targets_a1, targets_b1, lam1)  + (1-lam_scr) * mixup_criterion(criterion, outputs, targets_a2, targets_b2, lam2) #+ multi(perms_output, gen_labels)
   #     true_loss = criterion(outputs, targets) #+ multi(perms_output, gen_labels)
    ...
    #    correct += predicted.eq(targets).sum().item()
          correct += (lam_scr *(lam1 * predicted.eq(targets_a1.data).cpu().sum().float() + (1-lam1) * predicted.eq(targets_b1.data).cpu().sum().float()
                    ) + (1-lam_scr) * (lam2 * predicted.eq(targets_a2.data).cpu().sum().float() + (1 - lam2) * predicted.eq(targets_b2.data).cpu().sum().float())).item()
