...
trainloader = torch.utils.data.DataLoader(train_set, batch_size=opt.batch_size, shuffle=True, num_workers=opt.n_cpu)
...
random.shuffle(_shf)
optimizer_G = torch.optim.Adam(list(generator.parameters())+list(net.parameters()),lr=2e-4)#, betas=(opt.b1, opt.b2))#eps=1e-8)#, lr = 1e-6)#, lr=1e-4, betas=(opt.b1, opt.b2))
...
for epoch in range(opt.n_epochs):
 ...
 #       imgs = imgs.numpy().copy()
        ...
        if args.mixup == True:
          ...
          imgs[imgs.shape[0]//2:,:,:,:], targets_a, targets_b, lam = mixup_data(imgs1[imgs.shape[0]//2:,:,:,:], imgs2[imgs.shape[0]//2:,:,:,:], labels[imgs.size(0)//2:imgs.size(0)], 1, True)
       ...
       # save_image(gen_imgs2.data, opt.directory_name + "/train_real%d.png" % epoch, nrow=5, normalize=True)

        ce = nn.CrossEntropyLoss()

        judge_real, _, output_real = discriminator(real_imgs.detach())
    ...
    generator.eval()
    net.eval()
#    state = { 'net': generator.state_dict()}
#    state2 = { 'net': discriminator.state_dict()}
##    if not os.path.isdir('checkpoint'):
 #     os.mkdir('checkpoint')
 #   torch.save(state,'./'+"checkpoint/geenrator")
 #   torch.save(state2,'./'+"checkpoint/discriminator")
    total_loss = 0
