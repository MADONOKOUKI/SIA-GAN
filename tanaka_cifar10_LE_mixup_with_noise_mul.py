...
from Blockwise_scramble_LE import blockwise_scramble,blockwise_decramble
...
def mixup_data(x1,  y, alpha=args.alpha, use_cuda=True):
...
#    stddev = 0.2
 #   mixed_x = lam * x1 + (1 - lam) * Variable(torch.randn(x1.size()).cuda() * stddev)
    y_a, y_b = y, y[index]
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        # block scrambling
        x_stack = blockwise_scramble(imgs1,perm[np.random.randint(0,args.num_of_keys)])
        imgs1 = np.transpose(x_stack,(0,3,1,2))
        inputs1 = torch.from_numpy(imgs1)
 #       inputs1 = gaussian(inputs1,True)
 #       print(inputs1==inputs)
 ##       print(imgs1==imgs)
#        exit()
        x_stack = blockwise_scramble(imgs2,perm[np.random.randint(0,args.num_of_keys)])
        imgs2 = np.transpose(x_stack,(0,3,1,2))
        inputs2 = torch.from_numpy(imgs2)
#        inputs2 = gaussian(inputs2,True)
   ...
   #     inputs1,  targets = inputs1.cuda(), targets.cuda()     
       # inputs1, inputs2, targets = inputs1.cuda(), inputs2.cuda(), targets.cuda()
        ...
        inputs = blockwise_decramble(inputs.numpy().astype('float32'),perm[np.random.randint(0,args.num_of_keys)])
